...
public class LibraryControllerTest extends CardanoBlockchainTest {
    ...
    public void upgradeProxyContract() throws ApiException, CborSerializationException, CborDeserializationException, InterruptedException, CborException, AddressExcepion {
        ...
        // Reproduce production: after the deploy is indexed the library UTxO
        // caches are invalidated, so the upgrade build must lazily reload them
        // rather than read a stale (null) field.
        invalidateLibraryUtxoCache();
    ...
    private void invalidateLibraryUtxoCache() {
        for (String fieldName : new String[]{"proxyLibraryUtxo", "stateLibraryUtxo"}) {
            try {
                java.lang.reflect.Field field = LibraryService.class.getDeclaredField(fieldName);
                field.setAccessible(true);
                field.set(libraryService, null);
            } catch (ReflectiveOperationException exception) {
                throw new IllegalStateException("Failed to invalidate " + fieldName, exception);
            }
        }
    }
